...
**Inline agent annotations (`rift annotate`).** Let an agent leave review notes anchored to a file and line (or hunk), rendered inline in the diff preview — the agent explaining *why* it made a change, or a reviewing agent flagging a concern, right where the human is already reading. The design follows the reviewed-marks precedent, not a daemon: annotations live in a JSON store next to the marks (`.git/rift/annotations.json` — per worktree, uncommitted, worktree-aware for free), written by a plain composable command (`rift annotate <file> --line N -m "..."`, plus `--json` listing and a clear operation) and picked up live by `--watch`. Agent and human notes stay separate namespaces: an agent can list and clear its own notes but never edit a human's. Prior art: hunk drives the same feature through a loopback HTTP session daemon (`hunk session comment add`); rift deliberately does it with files and polling to stay transient, stateless, and scriptable — an agent that can run a CLI needs no protocol.

**Moved-line diff view (config).** git's `--color-moved=zebra` classifies relocated code and renders it in distinct colors — answering "moved or rewritten?" during refactor review. It cannot ship as part of the default git view because git silently ignores `--color-moved` when `--word-diff` is active (verified empirically), and word-level emphasis is the better everyday default. Ship it as a config option (e.g. `diff.git_view = "words" | "moved"` in config.toml) rather than a UI toggle — one best-effort default, configurable when someone asks.

**Mouse support.** Click to select a row, click a pane to focus it, wheel to scroll the preview — bubbletea already delivers the events. The care point is that enabling terminal mouse tracking captures the pointer, breaking native text selection (shift-click escape hatches vary by terminal emulator), and copy-from-the-diff matters in a review tool. So: wheel scrolling and click-to-select on by default, everything else conservative, and a config/env switch to disable tracking entirely.
